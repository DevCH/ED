...
while ($fila = mysql_fetch_object($result)) {
		...
		if (strlen($fila->xml)>0){
    			...
    			$pdf->Cell(4,$dif_y,$pdf->Write(6,$fila->xml,"http://ed.tabascoweb.com/php/01/fe/files/".$fila->xml),'R',0,'R');
		}else{
		...
		if (strlen($fila->pdf)>0){
    			...
    			$pdf->Cell(4,$dif_y,$pdf->Write(6,$fila->pdf,"http://ed.tabascoweb.com/php/01/fe/files/".$fila->pdf),'R',0,'R');
		}else{
    			$pdf->Cell(40,$dif_y,'','R',0,'R');
		}
		...
		$pdf->SetFont('','U');
    		...
    		$pdf->Write(6,str_pad($fila->idcli,'6','0',STR_PAD_LEFT),"http://ed.tabascoweb.com/php/01/docs/account-status.php?idcli=".$fila->idcli);
