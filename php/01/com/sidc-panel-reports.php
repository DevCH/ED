...
$(".asignaciones1 h2").html(och.cat[och.index].Catalogo); 
$(".asignaciones1").css("height", function() { return $(window).height()-och.height;});
...
function invocarFormulario(formData, jqForm, options) { 
	...
	var url = och.getValue(0)+"php/01/docs/"+$('select[name="formato"]').val();
	//alert(url);
...
function getDependencia(){
	...
	$.post(och.getValue(0)+"getEC/", { o:40, t:0, p:0 },
 		function(json){
...
function getProdSel(id){
	$.post(och.getValue(0)+"getEC/", { o:40, t:1, p:0, c:' idprodgpo = '+id },
 		function(json){
			if (json.length<=0) { return false;}
...
function getOrigen(){
	var lbl;
	$.post(och.getValue(0)+"getEC/", { o:40, t:2, p:0 },
 		function(json){
			if (json.length<=0) { return false;}
...
function getStatus(){
	var lbl;
	$.post(och.getValue(0)+"getEC/", { o:40, t:3, p:0 },
 		function(json){
			if (json.length<=0) { return false;}
...
function getPrioridad(){
	var lbl;
	$.post(och.getValue(0)+"getEC/", { o:40, t:4, p:0 },
 		function(json){
			if (json.length<=0) { return false;}
