...
<script type="text/javascript"> 
	...
	$("#users-contain").css("min-height",function(){
		var initHeight = $(window).height()-och.height;
...
function buscarContrato(responseText, statusText, xhr, $form){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:9, p:4,c:queryString },
		function(json){
			if (json.length==0) {
...
function tablaDeElementos(){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:12, p:3,c:" where idcontrato = "+idcontrato+" "+oBy+orderBy,from:oPag.currentPage, cantidad:oPag.CantidadPagina },
		function(json){
...
function cargarRegistrosLeidos(json){
	...
	$.each(json, function(i, item) {
		...
		var attach = 	och.getValue(0)+'php/01/fe/files/'+item.xml+","+och.getValue(0)+'php/01/fe/files/'+item.pdf;	
	...
	if(currentItem>-1){
		$(".detFac").find('#tr'+currentItem).each(function(){
			...
			if (confirm("Deseas eliminar este elemento?\n\n"+prod+' '+med)){
				...
				$('#cmdDelFac').hide();
				if (idfacdet>0){
					$.post(och.getValue(0)+"getEC/", { o:och.index, t:3, p:2,c:idfacdet },
					function(json){
...
function getRFC(idcli){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:0, p:0, c:'idcliparent='+idcli },
 		function(json){
			if (json.length<=0) { return false;}
...
function getContPAQi(idcli){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:1, p:0, c:'idcli='+idcli },
 		function(json){
			if (json.length<=0) { return false;}
...
function getProdCont(){
	var lbl;
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:100, p:0, c:'idcontrato='+idcontrato },
 		function(json){
			if (json.length<=0) { return false;}
...
function buscarFactura(idfactura){
	...
	indiceTabla=0;
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:13, p:3, c:' where idfactura='+idfactura },
 		function(json){
...
$("#viewInvoice").on('click',function(event){
	...
	if (isfe==0  ){
	   url = och.getValue(0)+"php/01/fe/viewInvoice1.php";
	}else{
	   	url = och.getValue(0)+"php/01/fe/files/"+arrItems[index].pdf;
	}
...
$("#printInvoiceXML1").on('click',function(event){
	...
	var queryString = $('#formBody').formSerialize();  
	var PARAMS = {data:queryString};  
	var url = och.getValue(0)+"php/01/fe/printInvoiceTest1.php";
...
function view_invoces_report(formData, jqForm, options) { 
	...
	var url = och.getValue(0)+"php/01/docs/invoices-report.php";
