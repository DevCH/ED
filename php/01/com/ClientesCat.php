...
<div id="tabs">
	...
	<fieldset>
		...
		<label for="nombre_rs" class="fieldset-margin-left">Nombre(s)</label>
	...
	<fieldset>
	   	...
	   	<label for="txtlocalidad" class="fieldset-margin-left">Delegación </label>
	...
	<fieldset>
	   	<label for="tel1">Tel&eacute;fono</label>
        	<input type="tel" name="tel1" id="tel1" value="(993)" class="text ui-widget-content ui-corner-all"/>

	   	<label for="cel1" class="fieldset-margin-left">Celular </label>
        	<input type="tel" name="cel1" id="cel1" value="(993)" class="text ui-widget-content ui-corner-all"/>


        	<label for="email" class="fieldset-margin-left">E-mail</label>
        	<input type="email" name="email" id="email" value="" class="text ui-widget-content ui-corner-all" pattern="^[a-z][\w.-]+@\w[\w.-]+\.[\w.-]*[a-z][a-z]$" />
	</fieldset>
	<fieldset>
...
<div id="toolbar" class="ui-widget-header ui-corner-all">
	...
	<span id="spanTitle">Listado de candidatos registrados</span>
</div>
	...
	<table id="tableList" class="table_base" >
		<thead>
			<tr class="ui-widget-header">
				...
				<th class="tel1">Tel&eacute;fono</th>
...
#tableList .tel1{width:120px !important;}
#tableList .cel1{width:120px !important;}
...
<script type="text/javascript"> 
	...
	var arOrderBy = [" idcandidato asc "," idcandidato desc "," nombrec asc "," nombrec desc "," tel1 asc "," tel1 desc "," cel1 asc "," cel1 desc "," email asc "," email desc "];
	...
	var xc = och.getMinHeight()-($("#formBody").height()+$("#toolbar").height()+60);
	...
	if(rol==1){
		$("#spanTitle").text("Registro de Propietarios");
		$("#tituloForm").text("Registro de Propietarios");
	}else{
		$("#spanTitle").text("Registro de Suplentes");
		$("#tituloForm").text("Registro de Suplentes");
	}
	...
	$("#refreshTable").on('click',function(event){
		event.preventDefault();
		oPag.currentPage=0;
		getPag2(idcandidato,"ed_candidatos","idcandidato", oPag,4,-1,20);
		tablaDeElementos();
	...
	$('#form-find-data-add-dom').submit(function() {
    		$(this).ajaxSubmit({success: invocarFormularioDomFis}); 
     	return false; 
	});
...
function tablaDeElementos(){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:21, p:4,c:oBy+orderBy,from:oPag.currentPage, cantidad:oPag.CantidadPagina   },
		function(json){
...
function tablaDeElementosFiltrados(form){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:22, p:4,c:oBy+orderBy+"|"+queryString,from:oPag.currentPage, cantidad:oPag.CantidadPagina   },
		function(json){
...
function cargarRegistrosLeidos(json){
	...
	$.each(json, function(i, item) {
		var id=item.idcandidato;

		arrItems[i] = {idcandidato:item.idcandidato, idcalle:item.idcalle, num_externo:item.num_externo, 
					...
					cel1:item.cel1, email:item.email, calle:item.calle, colonia:item.colonia, ciudad:item.ciudad, 
					localidad:item.localidad,municipio:item.municipio, estado:item.estado, pais:item.pais, 
					app_rs:item.app, apm_rs:item.apm, nombre_rs:item.nombre, nombrec:item.nombrec, 
					representante:item.representante,sexo:item.sexo, csexo:item.csexo
					};
		var str = "";
		...
		str +='<td><span>'+item.email+'</span></td>'; 
		str +="</tr>";
...
function editarRegistroActual(){
	...
	idcandidato = parseInt(arrItems[index].idcandidato);

	$("#idcandidato").val(parseInt(arrItems[index].idcandidato));
...
function invocarFormularioDomFis(responseText, statusText, xhr, $form){
	var queryString = $form.formSerialize(); 
	
	
	$.post(och.getValue(0)+"getEC/", { o:och.index, c:queryString, t:agregadoen, p:1 },
 	function(json){
		var q = json[0].msg;
...
function getCategorias(){
	var lbl;
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:3, p:0 },
 		function(json){
