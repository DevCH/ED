...
function getFactory(event){
		...
		var x = $('#folio').val();
		$.post(och.getValue(0)+"getEC/", { o:och.index, t:0, p:0, c:" cfolio = '"+x+"' " },
 			function(json){
		...
		getCatFac(); $(".asignaciones1 h2").html(och.cat[och.index].Catalogo); 
...
function invocarFormulario(formData, jqForm, options) { 
	...
	var url = och.getValue(0)+"php/01/docs/rep-fac-1.php";
...
function prodgpo(){	
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:0, p:0 },
 		function(json){
			if (json.length<=0) { return false;}
			$("#prodgpo").html('');	
			$.each(json, function(i, item) {
