...
function buscarFactura(responseText, statusText, xhr, $form){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:14, p:4,c:queryString },
		function(json){
			//alert(json.length);
...
function tablaDeElementos(){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:15, p:4,c:q+" "+oBy+orderBy,from:oPag.currentPage, cantidad:oPag.CantidadPagina },
		function(json){
			//alert(json.length);
...
function getTipoPagos(){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:0, p:0, c:'' },
 		function(json){
			if (json.length<=0) { return false;}
...
function getPrintReceipt(){
	...
	var url = och.getValue(0)+"php/01/fe/view_corte_caja1.php";
...
function printReceiptPaymentCash(piding){
	...
	var url = och.getValue(0)+"php/01/fe/receipt_payment_cash.php";
