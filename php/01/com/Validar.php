...
function tablaDeElementos(responseText, statusText, xhr, $form){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:17, p:4,c:q+"|"+oBy+orderBy,from:oPag.currentPage, cantidad:oPag.CantidadPagina },
		function(json){
			//alert(json.length);
...
function cargarRegistrosLeidos(json){
	...
	$(".delCurrentItem").on('click',function(event){
			...
			//alert(ind+och.sep+iduser);
			$.post(och.getValue(0)+"getEC/", { o:och.index, t:2, p:2, c:ind+och.sep+iduser },
 				function(json){
					alert(json[0].msg);
				     if (json[0].msg=="OK"){
...
function sendData(){
	...
	$.post(och.getValue(0)+"getEC/", { o:och.index, t:0, p:2, c:items1+och.sep+items2+och.sep+items3+och.sep+fecha+och.sep+iduser },
 		function(json){
...
function getPrintReceipt(){
	...
	var ley = 'VALIDADOS'; 
	//$.post(och.getValue(0)+"php/01/fe/view_corte_caja1.php", { fecha:fec }, "json");
	//window.location.href = och.getValue(0)+"php/01/fe/view_corte_caja1.php?fecha="+fec;

	var PARAMS = {fecha:fec,label:ley};  
	var url = och.getValue(0)+"php/01/docs/list-esq1.php";
